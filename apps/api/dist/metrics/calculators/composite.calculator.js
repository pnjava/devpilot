...
exports.weightedComposite = weightedComposite;
exports.computeDeliveryHealthIndex = computeDeliveryHealthIndex;
exports.computeStoryFrictionScore = computeStoryFrictionScore;
exports.computeTeamRiskScore = computeTeamRiskScore;
 ...
 */
function weightedComposite(values, weights, invertKeys = new Set()) {
...
function computeDeliveryHealthIndex(inputs, weights) {
    const w = weights ?? shared_1.DEFAULT_DELIVERY_HEALTH_WEIGHTS;
...
function computeStoryFrictionScore(inputs, weights) {
    const w = weights ?? shared_1.DEFAULT_FRICTION_WEIGHTS;
...
// ── Team Risk Score ────────────────────────────────────────────
function computeTeamRiskScore(inputs, weights) {
    const w = weights ?? shared_1.DEFAULT_TEAM_RISK_WEIGHTS;
    // For risk: higher values = worse. Invert READY_AT_START_PCT (higher = better).
    const invert = new Set(['READY_AT_START_PCT']);
    return weightedComposite(inputs, w, invert);
}
//# sourceMappingURL=composite.calculator.js.map