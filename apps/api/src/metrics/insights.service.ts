...
import type { TeamMetrics, StoryMetrics, NarrativeInsight, MetricScope } from '@devpilot/shared';

interface InsightRule {
  check: (m: TeamMetrics) => boolean;
  template: string;
  severity: 'INFO' | 'WARNING' | 'CRITICAL';
  metrics: (m: TeamMetrics) => Record<string, number>;
}

const TEAM_INSIGHT_RULES: InsightRule[] = [
  {
    check: (m) => m.blockedRatio > 0.3 && m.averageChurnScore > 40,
    template:
      '{teamName} is slowed more by blocker time ({blockedRatio}% of cycle) and requirement churn (avg score {churnScore}) than by coding throughput ({throughput} items completed).',
    severity: 'WARNING',
    metrics: (m) => ({
  ...
  },
  {
    ...
    template:
      '{teamName} has a high escaped bug rate ({escapedBugRate}%) relative to delivery volume, indicating quality issues reaching production.',
    severity: 'CRITICAL',
  ...
  },
  {
    ...
    template:
      'Only {readyPct}% of stories in {teamName} were ready when work started. This correlates with higher rework and review delays.',
    severity: 'WARNING',
  ...
  },
  {
    ...
    template:
      '{teamName} has low knowledge capture ({captureRate}%). Many completed stories lack linked documentation, increasing handoff risk.',
    severity: 'INFO',
  ...
  },
  {
    ...
    template:
      '{teamName} has {wipLoad} items in progress simultaneously, which may indicate context-switching overhead.',
    severity: 'WARNING',
  ...
  },
  {
    ...
    template:
      '{teamName} has {tracePct}% commit traceability — a significant portion of engineering work cannot be tied to specific stories.',
    severity: 'WARNING',
  ...
  },
  {
    ...
    template:
      '{teamName} has a {reopenRate}% reopen rate, suggesting stories are being marked done prematurely or acceptance criteria are unclear.',
    severity: 'WARNING',
...
export class InsightsService {
  generateTeamInsights(metrics: TeamMetrics): NarrativeInsight[] {
  ...
  generateStoryInsight(metrics: StoryMetrics): NarrativeInsight[] {
    ...
    if (
      metrics.clarificationIntensity > 50 &&
